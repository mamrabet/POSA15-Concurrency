...
public class PlayPingPong implements Runnable {
    ...
    private Handler[] msgHandle = new Handler[2];
    /**
    ...
    class PingPongThread extends HandlerThread implements Handler.Callback {
        ...
        protected void onLooperPrepared() {
            ...
            if (mMyType.equals(PingPong.PING)) {
                msgHandle[0] = new Handler(this);
            } else {
                msgHandle[1] = new Handler(this);
            }
            ...
            // @@ TODO - you fill in here.
            if (mMyType.equals(PingPong.PING)) {
                Handler pingHandler = msgHandle[0];
                Handler pongHandler = msgHandle[1];
                Message msgSend = pingHandler.obtainMessage(0, pongHandler);
                pingHandler.sendMessage(msgSend);
            }
        }
        ...
        public boolean handleMessage(Message reqMsg) {
            ...
            // appropriate code.
            PingPong isPing = PingPong.values()[reqMsg.what];

            if (mIterationsCompleted < mMaxIterations) {
             int count = mIterationsCompleted + 1;
                mOutputStrategy.print(isPing.name() + "(" + count + ")");
            ...
            // the reply.
            // @@ TODO - you fill in here.
            Handler msgSendHandle = (Handler) reqMsg.obj;
            int rank;
            if (isPing.equals(PingPong.PING)) {
                rank = 1;
            } else {
                rank = 0;
            }
            Message msg = msgSendHandle.obtainMessage(rank,msgHandle[rank]);
            // Return control to the Handler in the other
            // HandlerThread, which is the "target" of the msg
            // parameter.
            // @@ TODO - you fill in here.
            msgSendHandle.sendMessage(msg);
            return true;
        }
    }
